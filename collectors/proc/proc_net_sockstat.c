...
int32_t init_collector_proc_net_sockstat()
{
	...
	__metric_sockstat_tcp_mem =
		prom_collector_registry_must_register_metric(prom_gauge_new(
			...
			"Recv-Q = 0, Send-Q ≈400)",
			1, (const char *[]){ "sockstat" }));
	__metric_sockstat_udp_inuse =
		prom_collector_registry_must_register_metric(
			prom_gauge_new("node_sockstat_UDP_inuse",
				       "the number of UDP sockets in use", 1,
				       (const char *[]){ "sockstat" }));
	__metric_sockstat_udp_mem =
		prom_collector_registry_must_register_metric(
			prom_gauge_new("node_sockstat_UDP_mem",
				       "IPv4 UDP Sockets Memory, unit pages", 1,
				       (const char *[]){ "sockstat" }));
	__metric_sockstat_raw_inuse =
		prom_collector_registry_must_register_metric(prom_gauge_new(
			"node_sockstat_RAW_inuse", "IPv4 RAW Sockets inuse", 1,
			(const char *[]){ "sockstat" }));
	__metric_sockstat_frag_memory =
		prom_collector_registry_must_register_metric(
			prom_gauge_new("node_sockstat_FRAG_memory",
				       "IPv4 FRAG Sockets Memory, KiB", 1,
				       (const char *[]){ "sockstat" }));
	__metric_sockstat_frag_inuse =
		prom_collector_registry_must_register_metric(
			prom_gauge_new("node_sockstat_FRAG_inuse",
				       "Number of IP segments used", 1,
				       (const char *[]){ "sockstat" }));
	...
	__metric_sockstat_tcp_max_orphans =
		prom_collector_registry_must_register_metric(prom_gauge_new(
			"node_tcp_max_orphans", "IPv4 TCP Sockets Max Orphans",
			1, (const char *[]){ "sockstat" }));
				    ...
				    usec_t UNUSED(dt), const char *config_path)
{
	...
	if (unlikely(!__cfg_proc_net_socksat_filename)) {
		__cfg_proc_net_socksat_filename =
			appconfig_get_member_str(config_path, "monitor_file",
						 __proc_net_socksat_filename);
	}

	if (unlikely(!__pf_net_sockstat)) {
		__pf_net_sockstat =
			procfile_open(__cfg_proc_net_socksat_filename,
	...
	for (size_t l = 0; l < lines; l++) {
		...
		while (w + 1 < words) {
			const char *name =
				procfile_lineword(__pf_net_sockstat, l, w);
			w++;
			const char *value =
				procfile_lineword(__pf_net_sockstat, l, w);
			w++;
	...
	prom_gauge_set(__metric_sockstat_tcp_inuse,
		       __proc_net_sockstat.tcp_inuse,
		       (const char *[]){ "tcp" });
	prom_gauge_set(__metric_sockstat_tcp_orphan,
		       __proc_net_sockstat.tcp_orphan,
		       (const char *[]){ "tcp" });
	prom_gauge_set(__metric_sockstat_tcp_tw, __proc_net_sockstat.tcp_tw,
		       (const char *[]){ "tcp" });
	prom_gauge_set(__metric_sockstat_tcp_alloc,
		       __proc_net_sockstat.tcp_alloc,
		       (const char *[]){ "tcp" });
	prom_gauge_set(__metric_sockstat_tcp_mem,
		       __proc_net_sockstat.tcp_mem * pg_size_kb,
		       (const char *[]){ "tcp" });

	prom_gauge_set(__metric_sockstat_udp_inuse,
		       __proc_net_sockstat.udp_inuse,
		       (const char *[]){ "udp" });
	prom_gauge_set(__metric_sockstat_udp_mem,
		       __proc_net_sockstat.udp_mem * pg_size_kb,
		       (const char *[]){ "udp" });

	prom_gauge_set(__metric_sockstat_raw_inuse,
		       __proc_net_sockstat.raw_inuse,
		       (const char *[]){ "raw_sockets" });

	prom_gauge_set(__metric_sockstat_frag_inuse,
		       __proc_net_sockstat.frag_inuse,
