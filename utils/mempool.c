...
static struct xm_mempool_block_s *xm_memblock_create(uint32_t unit_size,
						     int32_t unit_count)
{
	struct xm_mempool_block_s *block = NULL;

	block = (struct xm_mempool_block_s *)malloc(
		sizeof(struct xm_mempool_block_s) + unit_size * unit_count);
	...
	block->next = NULL;
	// 只有 malloc 调用该函数，第一个 unit 已经分配出去了，所以 free_unit_pos = 1, free_unit_count 也减一
...
struct xm_mempool_s *xm_mempool_init(uint32_t unit_size,
				     int32_t init_mem_unit_count,
				     int32_t grow_mem_unit_count)
{
	struct xm_mempool_s *pool = NULL;

	if (unlikely(init_mem_unit_count <= 0 || grow_mem_unit_count <= 0)) {
		error("invalid init_mem_unit_count(%d) or grow_mem_unit_count(%d)",
		      init_mem_unit_count, grow_mem_unit_count);
		return NULL;
	}
	...
	// unit_size 必须大于 sizeof(int32_t)，这里存放下一个 free unit 的下标
	...
	debug("mempool init ok! unit_size: %d, init_mem_unit_count: %d, grow_mem_unit_count: %d",
	      pool->unit_size, pool->init_mem_unit_count,
...
void *xm_mempool_malloc(struct xm_mempool_s *pool)
{
	...
	if (unlikely(NULL == block)) {
		block = xm_memblock_create(pool->unit_size,
					   pool->init_mem_unit_count);
		if (unlikely(NULL == block)) {
	...
	// 查找一个有空闲 unit 的 block
	while (block && block->free_unit_count == 0) {
		block = block->next;
	}

	if (likely(block)) {
		// 找到一个空闲 unit 在 block 中
		uint32_t curr_free_pos = block->free_unit_pos;
		ret_ptr = (void *)((char *)block->data +
				   curr_free_pos * pool->unit_size);
...
int32_t xm_mempool_free(struct xm_mempool_s *pool, void *pfree)
{
	...
	// block 开始回收
	block->free_unit_count++;
	// 设置 pfree 的 next_free_unit_pos
	*((uint32_t *)pfree) = block->free_unit_pos;
	// 设置 block 的 free_unit_pos 为 pfree 的下标
	block->free_unit_pos =
		((char *)pfree - block->data) / (pool->unit_size);
	...
	if (block->block_size == block->free_unit_count * pool->unit_size) {
		// 如果 block 完全空闲
		if (pool->curr_mem_block_count > 1 &&
		    pool->root->free_unit_count) {
			// 如果 pool 有多个 block，且第一个 block 有空闲 unit
			if (block == pool->root) {
				// 如果要回收的 block 是第一个 block
		...
		// 如果 pool 的第一个 block 没有空闲 unit，这把这个 block 放到 pool 的第一个 block
		temp_block->next = block->next;
		block->next = pool->root;
		pool->root = block;
	}
...
void xm_print_mempool_info(struct xm_mempool_s *pool)
{
	...
	debug("*\tunit_size = %u\n*\tcurr_mem_block_count = %d\n*\tinit_mem_unit_count = %d\n*\t"
	      ...
	      pool->unit_size, pool->curr_mem_block_count,
	      pool->init_mem_unit_count, pool->grow_mem_unit_count);
	debug("-----------------------------------------------");

	block = pool->root;
	if (likely(NULL != block)) {
		for (; block != NULL; block = block->next) {
			debug("-----------------mempool block info[%d]-----------------",
			      i++);
			debug("*\tblock = %p\n*\tblock_size = %u\n*\tfree_unit_count = %u\n*\tfree_unit_pos = "
			      "%u\n*\tdata = %p",
			      (void *)block, block->block_size,
			      block->free_unit_count, block->free_unit_pos,
			      block->data);
			debug("-----------------------------------------------");
		}
	}
...
void xm_print_mempool_block_info_by_pointer(struct xm_mempool_s *pool,
					    void *ptr)
{
	struct xm_mempool_block_s *block = NULL;
	...
	if (unlikely(NULL == block)) {
		...
		debug("*\tblock = %p\n*\tblock_size = %u\n*\tfree_unit_count = %u\n*\tfree_unit_pos = "
		      "%u\n*\tdata = %p",
		      (void *)block, block->block_size, block->free_unit_count,
		      block->free_unit_pos, block->data);
		debug("-----------------------------------------------");
	}
